...
## mutli segmentation add
 ...
 #1       right_probs = F.sigmoid(output_right)
        ####left_probs = F.upsample(left_probs, size=(img_height, img_height))
 #1       right_probs = F.upsample(right_probs, size=(img_height, img_height))
##
        pourresize=torch.nn.Upsample((572,572))
        right_probs=pourresize(output_right)
        print('fct du haut')
        print(right_probs.shape)
        ####left_mask_np = left_probs.squeeze().cpu().numpy()
        right_mask_np = right_probs.squeeze().cpu().numpy()
        print(right_mask_np.shape)
        print(right_mask_np)
...
def mask_to_image(mask):
    return Image.fromarray((mask * 70).astype(np.uint8))
...
#lire txt
        img=Image.fromarray(normalize(np.loadtxt(fn)))
...
##
         #   print(np.where(mask[0]))
            ...
            for i in range(0,4):
                ...
                filename = str(i)+"train.png"
                result.save(filename)
  #              with np.printoptions(threshold=np.inf):
                print(np.where(mask[i]))
                allmasks=allmasks+(i)*mask[i]
            print(allmasks[:,300])
